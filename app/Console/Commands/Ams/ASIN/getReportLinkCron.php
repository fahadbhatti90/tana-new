...
class getReportLinkCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getasinreportlink:spasin';
    ...
    public function handle()
    {
        ...
        $getProfileReportIdList = Profile::with(['getSPAsinReportId'])->get();
        if (!empty($getProfileReportIdList)) {
            // get Specific Report Type ID
            foreach ($getProfileReportIdList as $profile) {
                if ($profile->getSPAsinReportId != NULL) {
                    foreach ($profile->getSPAsinReportId as $reportID) {
                        $reTry = 3;
                        a:
                        if ($reTry > 0) {
                            $reTry--;
                            ...
                            if ($profileInfo->is_active == 1) {
                                ...
                                try {
                                    ...
                                    // $response = $client->request('GET', $url . '/' . $profile->getSPAsinReportId->report_id, [
                                    ...
                                    if (!empty($body) && $body != null) {
                                        ...
                                        if ($body->status != 'IN_PROGRESS') {
                                            $SPAsinReportId = SPAsinReportId::find($reportID->id);
                                            if ($SPAsinReportId->is_done == 0) {
                                                $ReportData = new SPAsinReportLink();
                                                ...
                                                $SPAsinReportId->is_done = 1;
                                                $SPAsinReportId->save();
                                            } // end if
                                        } // end if

                                        // store report status
                                        Tracker::insertTrackRecord('Report name : Asin SP Report Link' . ' Profile id: ' . $profile->id . ', Report Date: ' . $reportID->report_date, 'record found');
                                    } else {
                                        // store report status
                                        Tracker::insertTrackRecord('Report name : Asin SP Report Link' . ' profile id: ' . $profile->id . ', Report Date: ' . $reportID->report_date, 'not record found');
                                    }
                                } catch (\Exception $ex) {
                                    if ($ex->getCode() == 401) {
                                        if (strstr($ex->getMessage(), 'Not authorized to access this advertiser')) {
                                            ...
                                            Log::error('Refresh Access token. In file filePath:Commands\Ams\ASIN\SP\getReportLinkCron');
                                    ...
                                    // store report status
                                    // store report status
                                    Tracker::insertTrackRecord('Asin SP Report Link', 'fail');
