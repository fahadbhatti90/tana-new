...
class getReportLinkCron extends Command
{
    ...
    public function handle()
    {
        ...
        $getProfileReportIdList = Profile::with(['getSBAdGroupReportId'])->get();
        if (!empty($getProfileReportIdList)) {
            // get Specific Report Type ID
            foreach ($getProfileReportIdList as $profile) {
                if ($profile->getSBAdGroupReportId != NULL) {
                    foreach ($profile->getSBAdGroupReportId as $reportID) {
                        $reTry = 3;
                        a:
                        if ($reTry > 0) {
                            $reTry--;
                                ...
                                try {
                                    ...
                                    if (!empty($body) && $body != null) {
                                        ...
                                        if ($body->status != 'IN_PROGRESS') {
                                            $SBAdGroupReportId = SBAdGroupReportId::find($reportID->id);
                                            if ($SBAdGroupReportId->is_done == 0) {
                                                $ReportData = new SBAdGroupReportLink();
                                                ...
                                                $SBAdGroupReportId->is_done = 1;
                                                $SBAdGroupReportId->save();
                                            } // end if
                                        }  // end if
                                        // store report status
                                        Tracker::insertTrackRecord('Report name : AdGroup SB Report Link' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportID->report_date, 'record found');
                                    } else {
                                        // store report status
                                        Tracker::insertTrackRecord('Report name : AdGroup SB Report Link' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportID->report_date, 'not record found');
                                    }
                                } catch (\Exception $ex) {
                                    if ($ex->getCode() == 401) {
                                    ...
                                    // store report status
                                    // store report status
                                    Tracker::insertTrackRecord('AdGroup SB Report Link', 'fail');
