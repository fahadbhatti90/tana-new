...
namespace App\Console\Commands\Ams\AdGroup\SD\Audiences;

use App\Model\Ams\AdGroup\SD\Audiences\SdAudiencesAdGroupReportId;
...
class getReportIdCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getSdAdGroupReportId:sdAudiencesAdGroup {daysBack=1} {recoverBackFromDate=0}';
    ...
    public function handle()
    {
        ...
        $profileList = Profile::with(['getTokenDetail', 'getAllSdAudienceAdGroupReportId', 'getAllSdAudienceAdGroupReportLink'])
            ->where('is_active', '1')
        ...
        $url = Config::get('amsconstants.amsApiUrl') . '/' . Config::get('amsconstants.apiVersion') . '/' . Config::get('amsconstants.SDadGroupsReport');

        foreach ($profileList as $profile) {
            $daysBackForProfile = $daysBack;
            if ($profile->sponsored_display_adgroup_sixty_days == 0 && $recoverBack == 0) {
                $daysBack = 60;
                $profile->sponsored_display_adgroup_sixty_days = 1;
                $profile->save();
            }
            ...
            for ($i = 0; $i < sizeof($reportDays); $i++) {
                ...
                $existReportId = $profile->getAllSdAudienceAdGroupReportId->where('report_date', $reportDateSixtyDays)->first();
                $existReportLink = $profile->getAllSdAudienceAdGroupReportLink->where('report_date', $reportDateSixtyDays)->where('is_done', 3)->first();
                if (!isset($existReportId) || isset($existReportLink)) {
                            ...
                            try {
                                ...
                                $response = $client->request('POST', $url, [
                                    ...
                                    'json' => [
                                        ...
                                        'metrics' => Config::get('amsconstants.sdAdGroupMetrics')
                                    ],
                                ...
                                if (!empty($body) && $body != null) {
                                    ...
                                    // store report status
                                    Tracker::insertTrackRecord('Report name : AdGroup SD Report Id' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportDateSixtyDays, 'record found');
                                } else {
                                    // store report status
                                    Tracker::insertTrackRecord('Report name : AdGroup SD Report Id' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportDateSixtyDays, 'not record found');
                                } //end else
                                if (!empty($DataArray)) {
                                    $reportId = new SdAudiencesAdGroupReportId();
                    ...
                    Log::info('ReportID Already exist in DB. Report name : AdGroup SD Report Id' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportDateSixtyDays);
                } // end else
            } //end for
        } // end foreach
