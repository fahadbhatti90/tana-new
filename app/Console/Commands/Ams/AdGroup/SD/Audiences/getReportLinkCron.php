...
use App\Model\Ams\AdGroup\SD\Audiences\SdAudiencesAdGroupReportLink;
...
class getReportLinkCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getSdAdgroupReportLink:sdAudiencesAdGroup';
    ...
    public function handle()
    {
        ...
        $getProfileReportIdList = Profile::with(['getSdAudienceAdGroupReportId'])->get();
        if (!empty($getProfileReportIdList)) {
            // get Specific Report Type ID
            foreach ($getProfileReportIdList as $profile) {
                if ($profile->getSdAudienceAdGroupReportId != NULL) {
                    foreach ($profile->getSdAudienceAdGroupReportId as $reportID) {
                        $reTry = 3;
                        a:
                        if ($reTry > 0) {
                            $reTry--;
                                ...
                                try {
                                    ...
                                    if (!empty($body) && $body != null) {
                                        ...
                                        if ($body->status != 'IN_PROGRESS') {
                                            $SPAdGroupReportId = SdAudiencesAdGroupReportId::find($reportID->id);
                                            if ($SPAdGroupReportId->is_done == 0) {
                                                $ReportData = new SdAudiencesAdGroupReportLink();
                                                ...
                                                $SPAdGroupReportId->is_done = 1;
                                                $SPAdGroupReportId->save();
                                            } // end if
                                        } // end if
                                        // store report status
                                        Tracker::insertTrackRecord('Report name : AdGroup SD Report Link' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportID->report_date, 'record found');
                                    } else {
                                        // store report status
                                        Tracker::insertTrackRecord('Report name : AdGroup SD Report Link' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportID->report_date, 'not record found');
                                    }
                                } catch (\Exception $ex) {
                                    if ($ex->getCode() == 401) {
                                    ...
                                    // store report status
                                    Tracker::insertTrackRecord('AdGroup SD Report Link', 'fail');
        ...
        Log::info("filePath:Commands\Ams\AdGroup\SD\getReportLinkCron. End Cron.");
    } // end handle
}
