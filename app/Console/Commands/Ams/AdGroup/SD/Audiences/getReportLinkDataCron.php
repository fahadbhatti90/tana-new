...
use App\Model\Ams\AdGroup\SD\Audiences\SdAudiencesAdGroupReport;
...
{
   //getCampaignReportId:sdAudiencesCampaign
    /**
     * The name and signature of the console command.
     *
     * @var string
     */
    protected $signature = 'getSdAdGroupReportLinkData:sdAudienceAdGroup';
    ...
    public function handle()
    {
        ...
        $getSdAdGroupReportLink = Profile::with(['getSdAudienceAdGroupReportLink'])->get();
        if (!empty($getSdAdGroupReportLink)) {
            // get Specific Report Type ID
            foreach ($getSdAdGroupReportLink as $profile) {
                if ($profile->getSdAudienceAdGroupReportLink != NULL) {
                    foreach ($profile->getSdAudienceAdGroupReportLink as $reportID) {
                        $reTry = 3;
                        a:
                        if ($reTry > 0) {
                            $reTry--;
                                    ...
                                    if (!empty($body) && $body != null) {
                                        ...
                                        if (!empty($DataArray)) {
                                            foreach (array_chunk($DataArray, 1000) as $data) {
                                                $report_data = new SdAudiencesAdGroupReport();
                                                $report_data->addReport($data);
                                            }
                                            $reportLink = SdAudiencesAdGroupReportLink::find($reportID->id);
