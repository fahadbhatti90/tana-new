...
class getReportLinkCron extends Command
{
    ...
    public function handle()
    {
        ...
        $getProfileReportIdList = Profile::with(['getSdAdGroupReportId'])->get();
        if (!empty($getProfileReportIdList)) {
            // get Specific Report Type ID
            foreach ($getProfileReportIdList as $profile) {
                if ($profile->getSdAdGroupReportId != NULL) {
                    foreach ($profile->getSdAdGroupReportId as $reportID) {
                        $reTry = 3;
                        a:
                        if ($reTry > 0) {
                            $reTry--;
                                ...
                                try {
                                    ...
                                    if (!empty($body) && $body != null) {
                                        ...
                                        if ($body->status != 'IN_PROGRESS') {
                                            $SPAdGroupReportId = SdAdGroupReportId::find($reportID->id);
                                            if ($SPAdGroupReportId->is_done == 0) {
                                                $ReportData = new SdAdGroupReportLink();
