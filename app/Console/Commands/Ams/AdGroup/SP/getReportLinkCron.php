...
class getReportLinkCron extends Command
{
    ...
    public function handle()
    {
        ...
        $getProfileReportIdList = Profile::with(['getSPAdGroupReportId'])->get();
        if (!empty($getProfileReportIdList)) {
            // get Specific Report Type ID
            foreach ($getProfileReportIdList as $profile) {
                if ($profile->getSPAdGroupReportId != NULL) {
                    foreach ($profile->getSPAdGroupReportId as $reportID) {
                        $reTry = 3;
                        a:
                        if ($reTry > 0) {
                            $reTry--;
                                ...
                                try {
                                    ...
                                    if (!empty($body) && $body != null) {
                                        ...
                                        if ($body->status != 'IN_PROGRESS') {
                                            $SPAdGroupReportId = SPAdGroupReportId::find($reportID->id);
                                            if ($SPAdGroupReportId->is_done == 0) {
                                                $ReportData = new SPAdGroupReportLink();
                                        ...
                                        // store report status
                                        Tracker::insertTrackRecord('Report name : AdGroup SP Report Link' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportID->report_date, 'record found');
                                    } else {
                                        // store report status
                                        Tracker::insertTrackRecord('Report name : AdGroup SP Report Link' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportID->report_date, 'not record found');
                                    }
                                } catch (\Exception $ex) {
                                    if ($ex->getCode() == 401) {
                                    ...
                                    // store report status
                                    // store report status
                                    Tracker::insertTrackRecord('AdGroup SP Report Link', 'fail');
