...
use App\Model\Ams\AdGroup\SP\SPAdGroupReport;
use App\Model\Ams\AdGroup\SP\SPAdGroupReportLink;
...
class getReportLinkDataCron extends Command
{
    ...
    public function handle()
    {
        ...
        $getSPAdGroupReportLink = Profile::with(['getSPAdGroupReportLink'])->get();
        if (!empty($getSPAdGroupReportLink)) {
            // get Specific Report Type ID
            foreach ($getSPAdGroupReportLink as $profile) {
                if ($profile->getSPAdGroupReportLink != NULL) {
                    foreach ($profile->getSPAdGroupReportLink as $reportID) {
                        $reTry = 3;
                        a:
                        if ($reTry > 0) {
                            $reTry--;
                                ...
                                try {
                                    ...
                                    if (!empty($body) && $body != null) {
                                        ...
                                        for ($i = 0; $i < $totalNumberOfRecords; $i++) {
                                            ...
                                            $storeArray['bid_plus'] = isset($body[$i]->bidPlus) ? 1 : 0;
                                        ...
                                        if (!empty($DataArray)) {
                                            foreach (array_chunk($DataArray, 1000) as $data) {
                                                $report_data = new SPAdGroupReport();
                                                $report_data->addReport($data);
                                            }
                                            $reportLink = SPAdGroupReportLink::find($reportID->id);
                                            $reportLink->is_done = 1;
                                            $reportLink->save();
                                        }
                                        // store report status
                                        Tracker::insertTrackRecord('report name : AdGroup SP Report Data' . ', Profile id: ' . $reportID->profile_id . ', Report Date: ' . $reportID->report_date, 'record found');
                                    } else {
                                        // store report status
                                        Tracker::insertTrackRecord('report name : AdGroup SP Report Data' . ', Profile id: ' . $reportID->profile_id . ', Report Date: ' . $reportID->report_date, 'not record found');
                                    }
                                } catch (\Exception $ex) {
                                    if ($ex->getCode() == 401) {
                    ...
                    log::info('report name : AdGroup SP Report Data' . ' profile id: ' . $profile->profile_id . ' Links record found');
                }// end else
            }// end foreach
        } else {
