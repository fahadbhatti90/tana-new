...
class getReportIdCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getCampaignReportId:sdAudiencesCampaign {daysBack=1} {recoverBackFromDate=0}';
     ...
     * @var string
     */
    protected $description = 'This command is used to get Audiences Campaign SD ReportId.';
    ...
    {
        Log::info("filePath:Commands\Ams\Campaign\SD\Audiences\getReportIdCron. Start Cron.");
        ...
        $profileList = Profile::with(['getTokenDetail', 'getAllSdAudiencesCampaignReportId', 'getAllSdAudiencesCampaignReportLink'])
            ->where('is_active', '1')
        ...
        foreach ($profileList as $profile) {
            ...
            for ($i = 0; $i < sizeof($reportDays); $i++) {
                ...
                $existReportId = $profile->getAllSdAudiencesCampaignReportId->where('report_date', $reportDateSixtyDays)->first();
                $existReportLink = $profile->getAllSdAudiencesCampaignReportLink->where('report_date', $reportDateSixtyDays)->where('is_done', 3)->first();
                    ...
                    if ($reTry > 0) {
                        ...
                        if ($profileInfo->is_active == 1) {
                            try {
                                ...
                                if (!empty($DataArray)) {
                                    $reportId = new SdAudiencesCampaignReportId();
