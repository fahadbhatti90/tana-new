...
use App\Model\Ams\Campaign\SD\Audiences\SdAudiencesCampaignReportId;
...
class getReportLinkCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getCampaignReportLink:sdAudiencesCampaign';
    ...
    public function handle()
    {
        ...
        $getProfileReportIdList = Profile::with(['getSdAudiencesCampaignReportId'])->get();

        if (!empty($getProfileReportIdList)) {
            // get Specific Report Type ID
            foreach ($getProfileReportIdList as $profile) {
                if ($profile->getSdAudiencesCampaignReportId != NULL) {
                    foreach ($profile->getSdAudiencesCampaignReportId as $reportID) {
                        $reTry = 3;
                        a:
                        if ($reTry > 0) {
                            $reTry--;
                                ...
                                try {
                                    ...
                                    if (!empty($body) && $body != null) {
                                        ...
                                        if ($body->status != 'IN_PROGRESS') {
                                            $SDCampaignReportId = SdAudiencesCampaignReportId::find($reportID->id);
                                            if ($SDCampaignReportId->is_done == 0) {
                                                $ReportData = new SdAudiencesCampaignReportLink();
                                                ...
                                                $SDCampaignReportId->is_done = 1;
                                                $SDCampaignReportId->save();
                                            } // end if
                                        } // end if

                                        // store report status
                                        Tracker::insertTrackRecord('Report name : Campaign SD Report Link' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportID->report_date, 'record found');
                                    } else {
                                        // store report status
                                        Tracker::insertTrackRecord('Report name : Campaign SD Report Link' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportID->report_date, 'not record found');
                                    }
                                } catch (\Exception $ex) {
                                    if ($ex->getCode() == 401) {
                                    ...
                                    // store report status
                                    Tracker::insertTrackRecord('Campaign SD Report Link', 'fail');
