...
class getCampaignList extends Command
{
     ...
     * @var string
     */
    protected $description = 'This command is used to get sponsored display campaign details';
    ...
    {
        Log::info("filePath:App\Console\Commands\Ams\Campaign\SD\getCampaignList. Start Cron.");
...
//                }
                if (!empty($allProfileIds)) {
                    foreach ($allProfileIds as $profile) {
                        ...
                        $url = Config::get('amsconstants.amsApiUrl') . '/' . Config::get('amsconstants.sdCampaignUrl');
                        $client = new Client();
                        // Goto Statement used
                        $reTry = 3;
                        a:
                                ...
                                try {
                                    ...
                                    if (!empty($responseBody) && !is_null($responseBody)) {
                                        ...
                                        foreach ($responseBody as $singleResponseRecord) {
                                            ...
                                            $campaign->type = 'SD';
                                            $campaign->campaign_type = 'sponsored_display';
                                        ...
                                        Campaign::updateCampaignRecords($profile->profile_id, 'SD', $DataActiveCampaignIDArray);
                                        unset($DataActiveCampaignIDArray);
                                        Tracker::insertTrackRecord('Report Name : Get SD Campaigns List Against' . ' Profile ID: ' . $profile->profile_id . ' Found', 'record found');
                                    } else {
                                        // campaign status
                                        Tracker::insertTrackRecord('Report Name : Get SD Campaigns List Against' . ' Profile ID: ' . $profile->profile_id . ' Not Found', 'not record found');
                                    } // end else
                                } catch (\Exception $ex) {
                                    if ($ex->getCode() == 401) {
                                    ...
                                    // store report status
                                    Tracker::insertTrackRecord('App\Console\Commands\Ams\Campaign\SD\getCampaignList', 'fail');
