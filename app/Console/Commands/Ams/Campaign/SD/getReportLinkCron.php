...
use App\Model\Ams\Campaign\SD\SdCampaignReportLink;
...
class getReportLinkCron extends Command
{
    ...
    public function handle()
    {
        ...
        $getProfileReportIdList = Profile::with(['getSdCampaignReportId'])->get();

        if (!empty($getProfileReportIdList)) {
            // get Specific Report Type ID
            foreach ($getProfileReportIdList as $profile) {
                if ($profile->getSdCampaignReportId != NULL) {
                    foreach ($profile->getSdCampaignReportId as $reportID) {
                        $reTry = 3;
                        a:
                        if ($reTry > 0) {
                            $reTry--;
                                ...
                                try {
                                    ...
                                    if (!empty($body) && $body != null) {
                                        ...
                                        if ($body->status != 'IN_PROGRESS') {
                                            $SDCampaignReportId = SdCampaignReportId::find($reportID->id);
                                            if ($SDCampaignReportId->is_done == 0) {
                                                $ReportData = new SdCampaignReportLink();
