...
use App\Model\Ams\Campaign\SD\SdCampaignReport;
...
class getReportLinkDataCron extends Command
{
    ...
    public function handle()
    {
        ...
        $getSdCampaignReportLink = Profile::with(['getSdCampaignReportLink'])->get();
        if (!empty($getSdCampaignReportLink)) {
            // get Specific Report Type ID
            foreach ($getSdCampaignReportLink as $profile) {
                if ($profile->getSdCampaignReportLink != NULL) {
                    foreach ($profile->getSdCampaignReportLink as $reportID) {
                        $reTry = 3;
                        a:
                        if ($reTry > 0) {
                            $reTry--;
                                    ...
                                    if (!empty($body) && $body != null) {
                                        ...
                                        if (!empty($DataArray)) {
                                            foreach (array_chunk($DataArray, 1000) as $data) {
                                                $report_data = new SdCampaignReport();
                                                $report_data->addReport($data);
                                            }
                                            $reportLink = SdCampaignReportLink::find($reportID->id);
