...
class getCampaignList extends Command
{
     ...
     * @var string
     */
    protected $description = 'This command is used to get sponsored product campaign details';
...
//                }
                if (!empty($allProfileIds)) {
                    foreach ($allProfileIds as $profile) {
                        ...
                        $url = Config::get('amsconstants.amsApiUrl') . '/' . Config::get('amsconstants.apiVersion') . '/' . Config::get('amsconstants.spCampaignUrl');
                        $client = new Client();
                        // Goto Statement used
                        $reTry = 3;
                        a:
                                ...
                                try {
                                    ...
                                    if (!empty($responseBody) && !is_null($responseBody)) {
                                        ...
                                        foreach ($responseBody as $singleResponseRecord) {
                                            ...
                                            $campaign->budget = isset($singleResponseRecord->budget) ? $singleResponseRecord->budget : '00.00';
                                            ...
                                            $campaign->targeting_type = $singleResponseRecord->targetingType;
                                            $campaign->premium_bid_adjustment = $singleResponseRecord->premiumBidAdjustment;
                                        ...
                                        // inactive campaigns
                                        Campaign::updateCampaignRecords($profile->profile_id, 'SP', $DataActiveCampaignIDArray);
                                        unset($DataActiveCampaignIDArray);
                                        Tracker::insertTrackRecord('Report Name : Get SP Campaigns List Against' . ' Profile ID: ' . $profile->profile_id . ' Found', 'record found');
                                    } else {
                                        // campaign status
                                        Tracker::insertTrackRecord('Report Name : Get SP Campaigns List Against' . ' Profile ID: ' . $profile->profile_id . ' Not Found', 'not record found');
                                    } // end else
                                } catch (\Exception $ex) {
                                    if ($ex->getCode() == 401) {
                                    ...
                                    // store report status
                                    Tracker::insertTrackRecord('App\Console\Commands\Ams\Campaign\SP\getCampaignList', 'fail');
