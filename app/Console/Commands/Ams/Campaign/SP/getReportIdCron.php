...
class getReportIdCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getcampaignreportid:spcampaign {daysBack=1} {recoverBackFromDate=0}';
    ...
    public function handle()
    {
        ...
        $profileList = Profile::with(['getTokenDetail', 'getAllSPCampaignReportId', 'getAllSPCampaignReportLink'])
            ->where('is_active', '1')
            ->where('is_sandbox_profile', '0')
            ->where('type', '<>', 'agency')
            ->get();

        $url = Config::get('amsconstants.amsApiUrl') . '/' . Config::get('amsconstants.apiVersion') . '/' . Config::get('amsconstants.SPCampaignReport');

        foreach ($profileList as $profile) {
            $daysBackForProfile = $daysBack;
            if ($profile->campaign_sp_sixty_days == 0 && $recoverBack == 0) {
                $daysBack = 60;
                $profile->campaign_sp_sixty_days = 1;
                $profile->save();
            }
            $reportDays = array();
            ...
            for ($i = 0; $i < sizeof($reportDays); $i++) {
                ...
                $existReportId = $profile->getAllSPCampaignReportId->where('report_date', $reportDateSixtyDays)->first();
                $existReportLink = $profile->getAllSPCampaignReportLink->where('report_date', $reportDateSixtyDays)->where('is_done', 3)->first();
                if (!isset($existReportId) || isset($existReportLink)) {
                            ...
                            try {
                                ...
                                $response = $client->request('POST', $url, [
                                    ...
                                    'json' => [
                                        ...
                                        'metrics' => Config::get('amsconstants.spCampaignMetrics'),
                                    ],
                                ...
                                if (!empty($body) && $body != null) {
                                    ...
                                    // store report status
                                    Tracker::insertTrackRecord('Report name : Campaign SP Report Id' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportDateSixtyDays, 'record found');
                                } else {
                                    // store report status
                                    Tracker::insertTrackRecord('Report name : Campaign SP Report Id' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportDateSixtyDays, 'not record found');
                                }//end else
                                if (!empty($DataArray)) {
                                    $reportId = new SPCampaignReportId();
                    ...
                    Log::info('ReportID Already exist in DB. Report name : Campaign SP Report Id' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportDateSixtyDays);
                } // end else
            } //end for
        } // end foreach
