...
use App\Model\Ams\Campaign\SP\SPCampaignReport;
...
class getReportLinkDataCron extends Command
{
    ...
    public function handle()
    {
        ...
        $getSPCampaignReportLink = Profile::with(['getSPCampaignReportLink'])->get();
        if (!empty($getSPCampaignReportLink)) {
            // get Specific Report Type ID
            foreach ($getSPCampaignReportLink as $profile) {
                if ($profile->getSPCampaignReportLink != NULL) {
                    foreach ($profile->getSPCampaignReportLink as $reportID) {
                        $reTry = 3;
                        a:
                        if ($reTry > 0) {
                            $reTry--;
                                ...
                                try {
                                    ...
                                    if (!empty($body) && $body != null) {
                                        ...
                                        for ($i = 0; $i < $totalNumberOfRecords; $i++) {
                                            ...
                                            $storeArray['bid_plus'] = ($body[$i]->bidPlus == true) ? 1 : 0;
                                        ...
                                        if (!empty($DataArray)) {
                                            foreach (array_chunk($DataArray, 1000) as $data) {
                                                $report_data = new SPCampaignReport();
                                                $report_data->addReport($data);
                                            }
                                            $reportLink = SPCampaignReportLink::find($reportID->id);
                                            $reportLink->is_done = 1;
                                            $reportLink->save();
                                        }
                                        // store report status
                                        Tracker::insertTrackRecord('report name : Campaign SP Report Data' . ', Profile id: ' . $reportID->profile_id . ', Report Date: ' . $reportID->report_date, 'record found');
                                    } else {
                                        // store report status
                                        Tracker::insertTrackRecord('report name : Campaign SP Report Data' . ', Profile id: ' . $reportID->profile_id . ', Report Date: ' . $reportID->report_date, 'not record found');
                                    }
                                } catch (\Exception $ex) {
                                    if ($ex->getCode() == 401) {
                    ...
                    Log::info('report name : Campaign SP Report Data' . ' profile id: ' . $profile->profile_id . ' Links record found');
                }// end else
            }// end foreach
        } else {
