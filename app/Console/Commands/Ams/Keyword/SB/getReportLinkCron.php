...
class getReportLinkCron extends Command
{
    ...
    public function handle()
    {
        ...
        $getProfileReportIdList = Profile::with(['getSBKeywordReportId'])->get();

        if (!empty($getProfileReportIdList)) {
            // get Specific Report Type ID
            foreach ($getProfileReportIdList as $profile) {
                if ($profile->getSBKeywordReportId != NULL) {
                    foreach ($profile->getSBKeywordReportId as $reportID) {
                        $reTry = 3;
                        a:
                        if ($reTry > 0) {
                            $reTry--;
                                ...
                                try {
                                    ...
                                    if (!empty($body) && $body != null) {
                                        ...
                                        if ($body->status != 'IN_PROGRESS') {
                                            $SBKeywordReportId = SBKeywordReportId::find($reportID->id);
                                            if ($SBKeywordReportId->is_done == 0) {
                                                $ReportData = new SBKeywordReportLink();
                                                ...
                                                $SBKeywordReportId->is_done = 1;
                                                $SBKeywordReportId->save();
                                            } // end if
                                        } // end if

                                        // store report status
                                        Tracker::insertTrackRecord('Report name : Keyword SB Report Link' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportID->report_date, 'record found');
                                    } else {
                                        // store report status
                                        Tracker::insertTrackRecord('Report name : Keyword SB Report Link' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportID->report_date, 'not record found');
                                    }
                                } catch (\Exception $ex) {
                                    if ($ex->getCode() == 401) {
                                    ...
                                    // store report status
                                    Tracker::insertTrackRecord('Keyword SB Report Link', 'fail');
