...
use App\Model\Ams\Keyword\SP\SPkeywordReportId;
...
class getReportIdCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getkeywordreportid:spkeyword {daysBack=1} {recoverBackFromDate=0}';
    ...
    public function handle()
    {
        ...
        $profileList = Profile::with(['getTokenDetail', 'getAllSPKeywordReportId', 'getAllSPKeywordReportLink'])
            ->where('is_active', '1')
            ->where('is_sandbox_profile', '0')
            ->where('type', '<>', 'agency')
            ->get();

        $url = Config::get('amsconstants.amsApiUrl') . '/' . Config::get('amsconstants.apiVersion') . '/' . Config::get('amsconstants.SPKeywordReport');

        foreach ($profileList as $profile) {
            $daysBackForProfile = $daysBack;
            if ($profile->keyword_sp_sixty_days == 0 && $recoverBack == 0) {
                $daysBack = 60;
                $profile->keyword_sp_sixty_days = 1;
                $profile->save();
            } // end if
            $reportDays = array();
            ...
            for ($i = 0; $i < sizeof($reportDays); $i++) {
                ...
                $existReportId = $profile->getAllSPKeywordReportId->where('report_date', $reportDateSixtyDays)->first();
                $existReportLink = $profile->getAllSPKeywordReportLink->where('report_date', $reportDateSixtyDays)->where('is_done', 3)->first();
                if (!isset($existReportId) || isset($existReportLink)) {
                            ...
                            try {
                                ...
                                $response = $client->request('POST', $url, [
                                    ...
                                    'json' => [
                                        ...
                                        'metrics' => Config::get('amsconstants.spKeywordMetrics'),
                                    ],
                                ...
                                if (!empty($body) && $body != null) {
                                    ...
                                    // store report status
                                    Tracker::insertTrackRecord('Report name : Keyword SP Report Id' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportDateSixtyDays, 'record found');
                                } else {
                                    // store report status
                                    Tracker::insertTrackRecord('Report name : Keyword SP Report Id' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportDateSixtyDays, 'not record found');
                                }//end else
                                if (!empty($DataArray)) {
                                    $reportId = new SPkeywordReportId();
                    ...
                    Log::info('ReportID Already exist in DB. Report name : Keyword SP Report Id' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportDateSixtyDays);
                } // end else
            } //end for
        } // end foreach
