...
use App\Model\Ams\Keyword\SP\SPkeywordReport;
use App\Model\Ams\Keyword\SP\SPkeywordReportLink;
...
class getReportLinkDataCron extends Command
{
    ...
    public function handle()
    {
        ...
        $getSPKeywordReportLink = Profile::with(['getSPKeywordReportLink'])->get();
        if (!empty($getSPKeywordReportLink)) {
            // get Specific Report Type ID
            foreach ($getSPKeywordReportLink as $profile) {
                if ($profile->getSPKeywordReportLink != NULL) {
                    foreach ($profile->getSPKeywordReportLink as $reportID) {
                        $reTry = 3;
                        a:
                        if ($reTry > 0) {
                            $reTry--;
                                    ...
                                    if (!empty($body) && $body != null) {
                                        ...
                                        if (!empty($DataArray)) {
                                            foreach (array_chunk($DataArray, 1000) as $data) {
                                                $report_data = new SPKeywordReport();
                                                $report_data->addReport($data);
                                            }
                                            $reportLink = SPkeywordReportLink::find($reportID->id);
                                            $reportLink->is_done = 1;
                                            $reportLink->save();
                                        }
                                        // store report status
                                        Tracker::insertTrackRecord('report name : Keyword SP Report Data' . ', Profile id: ' . $reportID->profile_id . ', Report Date: ' . $reportID->report_date, 'record found');
                                    } else {
                                        // store report status
                                        Tracker::insertTrackRecord('report name : Keyword SP Report Data' . ', Profile id: ' . $reportID->profile_id . ', Report Date: ' . $reportID->report_date, 'not record found');
                                    }
                                } catch (\Exception $ex) {
                                    if ($ex->getCode() == 401) {
                    ...
                    Log::info('report name : Keyword SP Report Data' . ' profile id: ' . $profile->profile_id . ' Links record found');
                }// end else
            }// end foreach
        } else {
