...
class getReportIdCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getSdProductsAdsReportId:sdAudiencesProductsAds {daysBack=1} {recoverBackFromDate=0}';
    ...
    public function handle()
    {
        ...
        $profileList = Profile::with(['getTokenDetail', 'getAllSdAudiencesProductAdsReportId', 'getAllSdAudiencesProductAdsReportLink'])
            ->where('is_active', '1')
        ...
        foreach ($profileList as $profile) {
            ...
            for ($i = 0; $i < sizeof($reportDays); $i++) {
                ...
                $existReportId = $profile->getAllSdAudiencesProductAdsReportId->where('report_date', $reportDateSixtyDays)->first();
                $existReportLink = $profile->getAllSdAudiencesProductAdsReportLink->where('report_date', $reportDateSixtyDays)->where('is_done', 3)->first();
                    ...
                    if ($reTry > 0) {
                        ...
                        if ($profileInfo->is_active == 1) {
                            try {
                                ...
                                if (!empty($DataArray)) {
                                    $reportId = new SdAudiencesProductAdsReportId();
