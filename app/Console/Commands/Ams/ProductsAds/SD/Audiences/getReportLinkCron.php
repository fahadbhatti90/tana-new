...
use App\Model\Ams\ProductAds\SD\Audiences\SdAudiencesProductAdsReportId;
...
class getReportLinkCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getSdProductsAdsReportLink:sdAudiencesProductsads';
    ...
    public function handle()
    {
        ...
        $getProfileReportIdList = Profile::with(['getSdAudiencesProductAdsReportId'])->get();
        if (!empty($getProfileReportIdList)) {
            // get Specific Report Type ID
            foreach ($getProfileReportIdList as $profile) {
                if ($profile->getSdAudiencesProductAdsReportId != NULL) {
                    foreach ($profile->getSdAudiencesProductAdsReportId as $reportID) {
                        $reTry = 3;
                        a:
                        if ($reTry > 0) {
                            $reTry--;
                                ...
                                try {
                                    ...
                                    if (!empty($body) && $body != null) {
                                        ...
                                        if ($body->status != 'IN_PROGRESS') {
                                            $SPProductAdsReportId = SdAudiencesProductAdsReportId::find($reportID->id);
                                            if ($SPProductAdsReportId->is_done == 0) {
                                                $ReportData = new SdAudiencesProductAdsReportLink();
                                        ...
                                        // store report status
                                        Tracker::insertTrackRecord('Report name : ProductAds SD Report Link' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportID->report_date, 'record found');
                                    } else {
                                        // store report status
                                        Tracker::insertTrackRecord('Report name : ProductAds SD Report Link' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportID->report_date, 'not record found');
                                    }
                                } catch (\Exception $ex) {
                                    if ($ex->getCode() == 401) {
                                        if (strstr($ex->getMessage(), 'Not authorized to access this advertiser')) {
                                            ...
                                            Log::error('Refresh Access token. In file filePath:Commands\Ams\ProductsAds\SD\getReportLinkCron');
                                    ...
                                    // store report status
                                    // store report status
                                    Tracker::insertTrackRecord('ProductAds SD Report Link', 'fail');
