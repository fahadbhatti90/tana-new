...
namespace App\Console\Commands\Ams\ProductsAds\SD\Audiences;

use App\Model\Ams\AuthToken;
use App\Model\Ams\ProductAds\SD\Audiences\SdAudiencesProductAdsReport;
use App\Model\Ams\ProductAds\SD\Audiences\SdAudiencesProductAdsReportLink;
...
class getReportLinkDataCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getSdProductsAdsReportLinkData:sdAudiencesProductsAds';
    ...
    public function handle()
    {
        ...
        $getSdProductAdsReportLink = Profile::with(['getSdAudiencesProductAdsReportLink'])->get();
        if (!empty($getSdProductAdsReportLink)) {
            // get Specific Report Type ID
            foreach ($getSdProductAdsReportLink as $profile) {
                if ($profile->getSdAudiencesProductAdsReportLink != NULL) {
                    foreach ($profile->getSdAudiencesProductAdsReportLink as $reportID) {
                        $reTry = 3;
                        a:
                        if ($reTry > 0) {
                            $reTry--;
                            ...
                            if ($profileInfo->is_active == 1) {
                                ...
                                try {
                                    ...
                                    $response = $client->request('GET', $url, [
                                        'headers' => [
                                            // 'Authorization' => 'Bearer ' . $profile->getSdProductAdsProfileList[0]->access_token,
                                            // 'Amazon-Advertising-API-ClientId' => $profile->getSdProductAdsProfileList[0]->client_id,
                                    ...
                                    if (!empty($body) && $body != null) {
                                        ...
                                        for ($i = 0; $i < $totalNumberOfRecords; $i++) {
                                            ...
                                            $storeArray['adId'] = isset($body[$i]->adId) ? $body[$i]->adId : '';
                                        ...
                                        if (!empty($DataArray)) {
                                            foreach (array_chunk($DataArray, 1000) as $data) {
                                                $report_data = new SdAudiencesProductAdsReport();
                                                $report_data->addReport($data);
                                            }
                                            $reportLink = SdAudiencesProductAdsReportLink::find($reportID->id);
                                            $reportLink->is_done = 1;
                                            $reportLink->save();
                                        }
                                        // store report status
                                        Tracker::insertTrackRecord('Report name : ProductAds SD Report Data' . ', Profile id: ' . $reportID->profile_id . ', Report Date: ' . $reportID->report_date, 'record found');
                                    } else {
                                        // store report status
                                        Tracker::insertTrackRecord('Report name : ProductAds SD Report Data' . ', Profile id: ' . $reportID->profile_id . ', Report Date: ' . $reportID->report_date, 'not record found');
                                    }
                                } catch (\Exception $ex) {
                                    if ($ex->getCode() == 401) {
                    ...
                    log::info('report name : ProductAds SD Report Data' . ' profile id: ' . $profile->profile_id . ' Links record found');
                } // end else
            } // end foreach
        } else {
