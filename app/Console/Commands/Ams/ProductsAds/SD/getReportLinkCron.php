...
class getReportLinkCron extends Command
{
    ...
    public function handle()
    {
        ...
        $getProfileReportIdList = Profile::with(['getSdProductAdsReportId'])->get();
        if (!empty($getProfileReportIdList)) {
            // get Specific Report Type ID
            foreach ($getProfileReportIdList as $profile) {
                if ($profile->getSdProductAdsReportId != NULL) {
                    foreach ($profile->getSdProductAdsReportId as $reportID) {
                        $reTry = 3;
                        a:
                        if ($reTry > 0) {
                            $reTry--;
                                ...
                                try {
                                    ...
                                    if (!empty($body) && $body != null) {
                                        ...
                                        if ($body->status != 'IN_PROGRESS') {
                                            $SPProductAdsReportId = SdProductAdsReportId::find($reportID->id);
                                            if ($SPProductAdsReportId->is_done == 0) {
                                                $ReportData = new SdProductAdsReportLink();
