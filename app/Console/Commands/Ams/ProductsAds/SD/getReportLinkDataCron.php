...
use App\Model\Ams\ProductAds\SD\SdProductAdsReport;
use App\Model\Ams\ProductAds\SD\SdProductAdsReportLink;
...
class getReportLinkDataCron extends Command
{
    ...
    public function handle()
    {
        ...
        $getSdProductAdsReportLink = Profile::with(['getSdProductAdsReportLink'])->get();
        if (!empty($getSdProductAdsReportLink)) {
            // get Specific Report Type ID
            foreach ($getSdProductAdsReportLink as $profile) {
                if ($profile->getSdProductAdsReportLink != NULL) {
                    foreach ($profile->getSdProductAdsReportLink as $reportID) {
                        $reTry = 3;
                        a:
                        if ($reTry > 0) {
                            $reTry--;
                                    ...
                                    if (!empty($body) && $body != null) {
                                        ...
                                        if (!empty($DataArray)) {
                                            foreach (array_chunk($DataArray, 1000) as $data) {
                                                $report_data = new SdProductAdsReport();
                                                $report_data->addReport($data);
                                            }
                                            $reportLink = SdProductAdsReportLink::find($reportID->id);
