...
class getReportIdCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getproductsadsreportid:productsads {daysBack=1} {recoverBackFromDate=0}';
    ...
    {
        Log::info("filePath:Commands\Ams\ProductAds\SP\getReportIdCron. Start Cron.");
        ...
        $profileList = Profile::with(['getTokenDetail', 'getAllSPProductAdsReportId', 'getAllSPProductAdsReportLink'])
            ->where('is_active', '1')
            ->where('is_sandbox_profile', '0')
            ->where('type', '<>', 'agency')
            ->get();

        $url = Config::get('amsconstants.amsApiUrl') . '/' . Config::get('amsconstants.apiVersion') . '/' . Config::get('amsconstants.productAdsReport');

        foreach ($profileList as $profile) {
            $daysBackForProfile = $daysBack;
            if ($profile->product_ads_sixty_days == 0 && $recoverBack == 0) {
                $daysBack = 60;
                $profile->product_ads_sixty_days = 1;
                $profile->save();
            } // end if


            $MetricListType = Config::get('amsconstants.productAdsMetricsSKU');
            if ($profile->type != 'seller') {
                $MetricListType = Config::get('amsconstants.productAdsMetrics');
            }
            $reportDays = array();
            ...
            for ($i = 0; $i < sizeof($reportDays); $i++) {
                ...
                $existReportId = $profile->getAllSPProductAdsReportId->where('report_date', $reportDateSixtyDays)->first();
                $existReportLink = $profile->getAllSPProductAdsReportLink->where('report_date', $reportDateSixtyDays)->where('is_done', 3)->first();
                if (!isset($existReportId) || isset($existReportLink)) {
                        ...
                        if ($profileInfo->is_active == 1) {
                            try {
                                ...
                                if (!empty($body) && $body != null) {
                                    ...
                                    // store report status
                                    Tracker::insertTrackRecord('Report name : ProductAds SP Report Id' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportDateSixtyDays, 'record found');
                                } else {
                                    // store report status
                                    Tracker::insertTrackRecord('Report name : ProductAds SP Report Id' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportDateSixtyDays, 'not record found');
                                }//end else
                                if (!empty($DataArray)) {
                                    $reportId = new SPProductAdsReportId();
                                ...
                                if ($ex->getCode() == 401) {
                                    if (strstr($ex->getMessage(), 'Not authorized to access this advertiser')) {
                                        ...
                                        Log::error('Refresh Access token. In file filePath:Commands\Ams\ProductAds\SP\getReportIdCron');
                    ...
                    Log::info('ReportID Already exist in DB. Report name : ProductAds SP Report Id' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportDateSixtyDays);
                } // end else
            } //end for
        } // end foreach
        Log::info("filePath:Commands\Ams\ProductAds\SP\getReportIdCron. End Cron.");
    } //end handle
}
