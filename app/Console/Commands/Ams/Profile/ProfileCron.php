...
class ProfileCron extends Command
{
    ...
    public function handle()
    {
        ...
        if (!$authToken->isEmpty()) {
            ...
            $url = Config::get('amsconstants.amsApiUrl') . '/' . Config::get('amsconstants.apiVersion') . '/' . Config::get('amsconstants.amsProfileUrl');
            foreach ($authToken as $singleToken) {
                ...
                try {
                    ...
                    if (!empty($body) && $totalValue > 0) {
                        ...
                        Profile::updateProfileRecords($DataActiveProfileIDArray);
                    } else {
                        // store status
