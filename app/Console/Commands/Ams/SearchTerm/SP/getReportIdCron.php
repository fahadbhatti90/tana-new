...
namespace App\Console\Commands\Ams\SearchTerm\SP;

use App\Model\Ams\AuthToken;
use App\Model\Ams\Profile;
use App\Model\Ams\SearchTerm\SP\SPSearchTermReportId;
...
class getReportIdCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getsearchtermreportid:spsearchterm {daysBack=1} {recoverBackFromDate=0}';
     ...
     * @var string
     */
    protected $description = 'This command is used to get Search Term SP reportId.';
    ...
    {
        Log::info("filePath:Commands\Ams\SearchTerm\SP\getReportIdCron. Start Cron.");
        ...
        $profileList = Profile::with(['getTokenDetail', 'getAllSPSearchTermReportId', 'getAllSPSearchTermReportLink'])
            ->where('is_active', '1')
            ->where('is_sandbox_profile', '0')
            ->where('type', '<>', 'agency')
            ->get();

        $url = Config::get('amsconstants.amsApiUrl') . '/' . Config::get('amsconstants.apiVersion') . '/' . Config::get('amsconstants.SPSearchTermReport');

        foreach ($profileList as $profile) {
            $daysBackForProfile = $daysBack;
            if ($profile->search_term_sp_sixty_days == 0 && $recoverBack == 0) {
                $daysBack = 60;
                $profile->search_term_sp_sixty_days = 1;
                $profile->save();
            } // end if
            $reportDays = array();
            ...
            for ($i = 0; $i < sizeof($reportDays); $i++) {
                ...
                $existReportId = $profile->getAllSPSearchTermReportId->where('report_date', $reportDateSixtyDays)->first();
                $existReportLink = $profile->getAllSPSearchTermReportLink->where('report_date', $reportDateSixtyDays)->where('is_done', 3)->first();
                if (!isset($existReportId) || isset($existReportLink)) {
                            ...
                            try {
                                ...
                                $response = $client->request('POST', $url, [
                                    ...
                                    'json' => [
                                        ...
                                        'metrics' => Config::get('amsconstants.spSearchTermMetrics'),
                                    ],
                                ...
                                if (!empty($body) && $body != null) {
                                    ...
                                    // store report status
                                    Tracker::insertTrackRecord('Report name : Search Term SP Report Id' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportDateSixtyDays, 'record found');
                                } else {
                                    // store report status
                                    Tracker::insertTrackRecord('Report name : Search Term SP Report Id' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportDateSixtyDays, 'not record found');
                                }//end else
                                if (!empty($DataArray)) {
                                    $reportId = new SPSearchTermReportId();
                                ...
                                if ($ex->getCode() == 401) {
                                    if (strstr($ex->getMessage(), 'Not authorized to access this advertiser')) {
                                        ...
                                        Log::error('Refresh Access token. In file filePath:Commands\Ams\SearchTerm\SP\getReportIdCron');
                    ...
                    Log::info('ReportID Already exist in DB. Report name : SearchTerm SP Report Id' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportDateSixtyDays);
                } // end else
            } //end for
        } // end foreach
        Log::info("filePath:Commands\Ams\SearchTerm\SP\getReportIdCron. End Cron.");
    } //end handle
}
