...
class getReportLinkCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getsearchtermreportlink:spsearchterm';
     ...
     * @var string
     */
    protected $description = 'This command is used to get Search Term SP report link location.';
    ...
    {
        Log::info("filePath:Commands\Ams\SearchTerm\SP\getReportLinkCron. Start Cron.");
        Log::info($this->description);
        setMemoryLimitAndExeTime();
        $getProfileReportIdList = Profile::with(['getSPSearchTermReportId'])->get();

        if (!empty($getProfileReportIdList)) {
            // get Specific Report Type ID
            foreach ($getProfileReportIdList as $profile) {
                if ($profile->getSPSearchTermReportId != NULL) {
                    foreach ($profile->getSPSearchTermReportId as $reportID) {
                        $reTry = 3;
                        a:
                        if ($reTry > 0) {
                            $reTry--;
                                ...
                                try {
                                    ...
                                    if (!empty($body) && $body != null) {
                                        ...
                                        if ($body->status != 'IN_PROGRESS') {
                                            $SPSearchTermReportId = SPSearchTermReportId::find($reportID->id);
                                            if ($SPSearchTermReportId->is_done == 0) {
                                                $ReportData = new SPSearchTermReportLink();
                                                ...
                                                $SPSearchTermReportId->is_done = 1;
                                                $SPSearchTermReportId->save();
                                            } // end if
                                        } // end if
                                        // store report status
                                        Tracker::insertTrackRecord('Report name : Search Term SP Report Link' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportID->report_date, 'record found');
                                    } else {
                                        // store report status
                                        Tracker::insertTrackRecord('Report name : Search Term SP Report Link' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportID->report_date, 'not record found');
                                    }
                                } catch (\Exception $ex) {
                                    if ($ex->getCode() == 401) {
                                        if (strstr($ex->getMessage(), 'Not authorized to access this advertiser')) {
                                            ...
                                            Log::error('Refresh Access token. In file filePath:Commands\Ams\SearchTerm\SP\getReportLinkCron');
                                    ...
                                    // store report status
                                    Tracker::insertTrackRecord('Search Term SP Report Link', 'fail');
