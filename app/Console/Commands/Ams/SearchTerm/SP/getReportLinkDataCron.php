...
use App\Model\Ams\SearchTerm\SP\SPSearchTermReport;
use App\Model\Ams\SearchTerm\SP\SPSearchTermReportLink;
...
class getReportLinkDataCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'getsearchtermreportlinkdata:spsearchterm';
     ...
     * @var string
     */
    protected $description = 'This command is used to get Search Term SP link data.';
    ...
    {
        Log::info("filePath:Commands\Ams\SearchTerm\SP\getReportLinkDataCron. Start Cron.");
        Log::info($this->description);
        setMemoryLimitAndExeTime();
        $getSPSearchTermReportLink = Profile::with(['getSPSearchTermReportLink'])->get();
        if (!empty($getSPSearchTermReportLink)) {
            // get Specific Report Type ID
            foreach ($getSPSearchTermReportLink as $profile) {
                if ($profile->getSPSearchTermReportLink != NULL) {
                    foreach ($profile->getSPSearchTermReportLink as $reportID) {
                        $reTry = 3;
                        a:
                        if ($reTry > 0) {
                            $reTry--;
                                    ...
                                    if (!empty($body) && $body != null) {
                                        ...
                                        if (!empty($DataArray)) {
                                            foreach (array_chunk($DataArray, 1000) as $data) {
                                                $report_data = new SPSearchTermReport();
                                                $report_data->addReport($data);
                                            }
                                            $reportLink = SPSearchTermReportLink::find($reportID->id);
                                            $reportLink->is_done = 1;
                                            $reportLink->save();
                                        }
                                        // store report status
                                        Tracker::insertTrackRecord('report name : Search Term SP Report Data' . ', Profile id: ' . $reportID->profile_id . ', Report Date: ' . $reportID->report_date, 'record found');
                                    } else {
                                        // store report status
                                        Tracker::insertTrackRecord('report name : Search Term SP Report Data' . ', Profile id: ' . $reportID->profile_id . ', Report Date: ' . $reportID->report_date, 'not record found');
                                    }
                                } catch (\Exception $ex) {
                                    if ($ex->getCode() == 401) {
                                        if (strstr($ex->getMessage(), 'Not authorized to access this advertiser')) {
                                            ...
                                            Log::error('Refresh Access token. In file filePath:Commands\Ams\SearchTerm\SP\getReportLinkDataCron');
                    ...
                    Log::info('report name : Search Term SP Report Data' . ' profile id: ' . $profile->profile_id . ' Links record found');
                }// end else
            }// end foreach
        } else {
            Log::info('Profile id not record found');
        }// end else
        Log::info("filePath:Commands\Ams\SearchTerm\SP\getReportLinkDataCron. End Cron.");
    }
}
