...
class getReportIdCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'gettargetreportid:sdTargetsAudience {daysBack=1} {recoverBackFromDate=0}';
     ...
     * @var string
     */
    protected $description = 'This command is used to get Audience Target SD reportId.';
    ...
    {
        Log::info("filePath:Commands\Ams\Target\SD\Audience\getReportIdCron. Start Cron.");
        ...
        $profileList = Profile::with(['getTokenDetail', 'getAllSdTargetAudienceReportId', 'getAllSdTargetAudienceReportLink'])
            ->where('is_active', '1')
        ...
        foreach ($profileList as $profile) {
            ...
            if ($profile->sponsored_display_audiences_targeting_sixty_days == 0 && $recoverBack == 0) {
                $daysBack = 60;
                $profile->sponsored_display_audiences_targeting_sixty_days = 1;
                $profile->save();
            } // end if
            ...
            for ($i = 0; $i < sizeof($reportDays); $i++) {
                ...
                $existReportId = $profile->getAllSdTargetAudienceReportId->where('report_date', $reportDateSixtyDays)->first();
                $existReportLink = $profile->getAllSdTargetAudienceReportLink->where('report_date', $reportDateSixtyDays)->where('is_done', 3)->first();
                if (!isset($existReportId) || isset($existReportLink)) {
                        ...
                        if ($profileInfo->is_active == 1) {
                            try {
                                ...
                                if (!empty($body) && $body != null) {
                                    ...
                                    // store report status
                                    Tracker::insertTrackRecord('Report name : Audience Target SD Report Id' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportDateSixtyDays, 'record found');
                                } else {
                                    // store report status
                                    Tracker::insertTrackRecord('Report name : Audience Target SD Report Id' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportDateSixtyDays, 'not record found');
                                } //end else
                                if (!empty($DataArray)) {
                                    $reportId = new SdTargetAudienceReportId();
                                ...
                                if ($ex->getCode() == 401) {
                                    if (strstr($ex->getMessage(), 'Not authorized to access this advertiser')) {
                                        ...
                                        Log::error('Refresh Access token. In file filePath:Commands\Ams\Target\SD\Audience\getReportIdCron');
                    ...
                    Log::info('ReportID Already exist in DB. Report name : Audience Target SD Report Id' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportDateSixtyDays);
                } // end else
            } //end for
        } // end foreach
        Log::info("filePath:Commands\Ams\Target\SD\Audience\getReportIdCron. End Cron.");
    } //end handle
}
