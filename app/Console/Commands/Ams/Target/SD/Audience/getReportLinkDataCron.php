...
use App\Model\Ams\Target\SD\Audience\SdTargetAudienceReport;
...
class getReportLinkDataCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'gettargetreportlinkdata:sdTargetsAudience';
     ...
     * @var string
     */
    protected $description = 'This command is used to get Audience Target SD report link data.';
    ...
    {
        Log::info("filePath:Commands\Ams\Target\SD\Audience\getReportLinkDataCron. Start Cron.");
        Log::info($this->description);
        setMemoryLimitAndExeTime();
        $getSdTargetReportLink = Profile::with(['getSdTargetAudienceReportLink'])->get();
        if (!empty($getSdTargetReportLink)) {
            // get Specific Report Type ID
            foreach ($getSdTargetReportLink as $profile) {
                if ($profile->getSdTargetAudienceReportLink != NULL) {
                    foreach ($profile->getSdTargetAudienceReportLink as $reportID) {
                        $reTry = 3;
                        a:
                        if ($reTry > 0) {
                            $reTry--;
                                    ...
                                    if (!empty($body) && $body != null) {
                                        ...
                                        if (!empty($DataArray)) {
                                            foreach (array_chunk($DataArray, 1000) as $data) {
                                                $report_data = new SdTargetAudienceReport();
                                                $report_data->addReport($data);
                                            }
                                            $reportLink = SdTargetAudienceReportLink::find($reportID->id);
                                            $reportLink->is_done = 1;
                                            $reportLink->save();
                                        }
                                        // store report status
                                        Tracker::insertTrackRecord('report name : Audience Target SD Report Data' . ', Profile id: ' . $reportID->profile_id . ', Report Date: ' . $reportID->report_date, 'record found');
                                    } else {
                                        // store report status
                                        Tracker::insertTrackRecord('report name : Audience Target SD Report Data' . ', Profile id: ' . $reportID->profile_id . ', Report Date: ' . $reportID->report_date, 'not record found');
                                    }
                                } catch (\Exception $ex) {
                                    if ($ex->getCode() == 401) {
                                        if (strstr($ex->getMessage(), 'Not authorized to access this advertiser')) {
                                            ...
                                            Log::error('Refresh Access token. In file filePath:Commands\Ams\Target\SD\Audience\getReportLinkDataCron');
                    ...
                    Log::info('report name : Target SD Report Data' . ' profile id: ' . $profile->profile_id . ' Links record found');
                } // end else
            } // end foreach
        } else {
            Log::info('Profile id not record found');
        } // end else
        Log::info("filePath:Commands\Ams\Target\SD\Audience\getReportLinkDataCron. End Cron.");
    }
}
