...
namespace App\Console\Commands\Ams\Target\SD\Product;

use App\Model\Ams\AuthToken;
use App\Model\Ams\Profile;
use App\Model\Ams\Target\SD\SdTargetReportId;
...
class getReportIdCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'gettargetreportid:sdTargets {daysBack=1} {recoverBackFromDate=0}';
     ...
     * @var string
     */
    protected $description = 'This command is used to get Product Target SD reportId.';
    ...
    {
        Log::info("filePath:Commands\Ams\Target\SD\Product\getReportIdCron. Start Cron.");
        ...
        $profileList = Profile::with(['getTokenDetail', 'getAllSdTargetReportId', 'getAllSdTargetReportLink'])
            ->where('is_active', '1')
        ...
        $url = Config::get('amsconstants.amsApiUrl') . '/' . Config::get('amsconstants.apiVersion') . '/' . Config::get('amsconstants.sdTargetsReport');

        foreach ($profileList as $profile) {
            $daysBackForProfile = $daysBack;
            if ($profile->sponsored_display_product_targeting_sixty_days == 0 && $recoverBack == 0) {
                $daysBack = 60;
                $profile->sponsored_display_product_targeting_sixty_days = 1;
                $profile->save();
            } // end if
            ...
            for ($i = 0; $i < sizeof($reportDays); $i++) {
                ...
                $existReportId = $profile->getAllSdTargetReportId->where('report_date', $reportDateSixtyDays)->first();
                $existReportLink = $profile->getAllSdTargetReportLink->where('report_date', $reportDateSixtyDays)->where('is_done', 3)->first();
                if (!isset($existReportId) || isset($existReportLink)) {
                            ...
                            try {
                                ...
                                $response = $client->request('POST', $url, [
                                    ...
                                    'json' => [
                                        ...
                                        'metrics' => Config::get('amsconstants.sdTargetingMetrics'),
                                    ],
                                ...
                                if (!empty($body) && $body != null) {
                                    ...
                                    // store report status
                                    Tracker::insertTrackRecord('Report name : Product Target SD Report Id' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportDateSixtyDays, 'record found');
                                } else {
                                    // store report status
                                    Tracker::insertTrackRecord('Report name : Product Target SD Report Id' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportDateSixtyDays, 'not record found');
                                } //end else
                                if (!empty($DataArray)) {
                                    $reportId = new SdTargetReportId();
                                ...
                                if ($ex->getCode() == 401) {
                                    if (strstr($ex->getMessage(), 'Not authorized to access this advertiser')) {
                                        ...
                                        Log::error('Refresh Access token. In file filePath:Commands\Ams\Target\SD\Product\getReportIdCron');
                    ...
                    Log::info('ReportID Already exist in DB. Report name : Product Target SD Report Id' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportDateSixtyDays);
                } // end else
            } //end for
        } // end foreach
        Log::info("filePath:Commands\Ams\Target\SD\Product\getReportIdCron. End Cron.");
    } //end handle
}
