...
class getReportLinkCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'gettargetreportlink:sdTargets';
     ...
     * @var string
     */
    protected $description = 'This command is used to get Product Target SD report link.';
    ...
    {
        Log::info("filePath:Commands\Ams\Target\SD\Product\getReportLinkCron. Start Cron.");
        Log::info($this->description);
        setMemoryLimitAndExeTime();
        $getProfileReportIdList = Profile::with(['getSdTargetReportId'])->get();
        if (!empty($getProfileReportIdList)) {
            // get Specific Report Type ID
            foreach ($getProfileReportIdList as $profile) {
                if ($profile->getSdTargetReportId != NULL) {
                    foreach ($profile->getSdTargetReportId as $reportID) {
                        $reTry = 3;
                        a:
                        if ($reTry > 0) {
                            $reTry--;
                                ...
                                try {
                                    ...
                                    if (!empty($body) && $body != null) {
                                        ...
                                        if ($body->status != 'IN_PROGRESS') {
                                            $SPTargetReportId = SdTargetReportId::find($reportID->id);
                                            if ($SPTargetReportId->is_done == 0) {
                                                $ReportData = new SdTargetReportLink();
                                        ...
                                        // store report status
                                        Tracker::insertTrackRecord('Report name : Product Target SD Report Link' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportID->report_date, 'record found');
                                    } else {
                                        // store report status
                                        Tracker::insertTrackRecord('Report name : Product Target SD Report Link' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportID->report_date, 'not record found');
                                    }
                                } catch (\Exception $ex) {
                                    if ($ex->getCode() == 401) {
                                        if (strstr($ex->getMessage(), 'Not authorized to access this advertiser')) {
                                            ...
                                            Log::error('Refresh Access token. In file filePath:Commands\Ams\Target\SD\Product\getReportLinkCron');
        ...
        Log::info("filePath:Commands\Ams\Target\SD\Product\getReportLinkCron. End Cron.");
    }
}
