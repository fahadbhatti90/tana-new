...
use App\Model\Ams\Target\SP\SPTargetReportId;
...
class getReportIdCron extends Command
{
     ...
     * @var string
     */
    protected $signature = 'gettargetreportid:targets {daysBack=1} {recoverBackFromDate=0}';
    ...
    {
        Log::info("filePath:Commands\Ams\Target\SP\getReportIdCron. Start Cron.");
        ...
        $profileList = Profile::with(['getTokenDetail', 'getAllSPTargetReportId', 'getAllSPTargetReportLink'])
            ->where('is_active', '1')
            ->where('is_sandbox_profile', '0')
            ->where('type', '<>', 'agency')
            ->get();

        $url = Config::get('amsconstants.amsApiUrl') . '/' . Config::get('amsconstants.apiVersion') . '/' . Config::get('amsconstants.targetsReport');

        foreach ($profileList as $profile) {
            $daysBackForProfile = $daysBack;
            if ($profile->product_targeting_sixty_days == 0 && $recoverBack == 0) {
                $daysBack = 60;
                $profile->product_targeting_sixty_days = 1;
                $profile->save();
            } // end if
            ...
            for ($i = 0; $i < sizeof($reportDays); $i++) {
                ...
                $existReportId = $profile->getAllSPTargetReportId->where('report_date', $reportDateSixtyDays)->first();
                $existReportLink = $profile->getAllSPTargetReportLink->where('report_date', $reportDateSixtyDays)->where('is_done', 3)->first();
                if (!isset($existReportId) || isset($existReportLink)) {
                            ...
                            try {
                                ...
                                $response = $client->request('POST', $url, [
                                    ...
                                    'json' => [
                                        ...
                                        'metrics' => Config::get('amsconstants.productTargetingMetrics'),
                                    ],
                                ...
                                if (!empty($body) && $body != null) {
                                    ...
                                    // store report status
                                    Tracker::insertTrackRecord('Report name : Target SP Report Id' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportDateSixtyDays, 'record found');
                                } else {
                                    // store report status
                                    Tracker::insertTrackRecord('Report name : Target SP Report Id' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportDateSixtyDays, 'not record found');
                                }//end else
                                if (!empty($DataArray)) {
                                    $reportId = new SPTargetReportId();
                                ...
                                if ($ex->getCode() == 401) {
                                    if (strstr($ex->getMessage(), 'Not authorized to access this advertiser')) {
                                        ...
                                        Log::error('Refresh Access token. In file filePath:Commands\Ams\Target\SP\getReportIdCron');
                    ...
                    Log::info('ReportID Already exist in DB. Report name : Target SP Report Id' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportDateSixtyDays);
                } // end else
            } //end for
        } // end foreach
        Log::info("filePath:Commands\Ams\Target\SP\getReportIdCron. End Cron.");
    } //end handle
}
