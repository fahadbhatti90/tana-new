...
class getReportLinkCron extends Command
{
    ...
    {
        Log::info("filePath:Commands\Ams\Target\SP\getReportLinkCron. Start Cron.");
        Log::info($this->description);
        setMemoryLimitAndExeTime();
        $getProfileReportIdList = Profile::with(['getSPTargetReportId'])->get();

        if (!empty($getProfileReportIdList)) {
            // get Specific Report Type ID
            foreach ($getProfileReportIdList as $profile) {
                if ($profile->getSPTargetReportId != NULL) {
                    foreach ($profile->getSPTargetReportId as $reportID) {
                        $reTry = 3;
                        a:
                        if ($reTry > 0) {
                            $reTry--;
                                ...
                                try {
                                    ...
                                    if (!empty($body) && $body != null) {
                                        ...
                                        if ($body->status != 'IN_PROGRESS') {
                                            $SPTargetReportId = SPTargetReportId::find($reportID->id);
                                            if ($SPTargetReportId->is_done == 0) {
                                                $ReportData = new SPTargetReportLink();
                                        ...
                                        // store report status
                                        Tracker::insertTrackRecord('Report name : Target SP Report Link' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportID->report_date, 'record found');
                                    } else {
                                        // store report status
                                        Tracker::insertTrackRecord('Report name : Target SP Report Link' . ', Profile id: ' . $profile->id . ', Report Date: ' . $reportID->report_date, 'not record found');
                                    }
                                } catch (\Exception $ex) {
                                    if ($ex->getCode() == 401) {
                                        if (strstr($ex->getMessage(), 'Not authorized to access this advertiser')) {
                                            ...
                                            Log::error('Refresh Access token. In file filePath:Commands\Ams\Target\SP\getReportLinkCron');
                                    ...
                                    // store report status
                                    Tracker::insertTrackRecord('Target SP Report Link', 'fail');
        ...
        Log::info("filePath:Commands\Ams\Target\SP\getReportLinkCron. End Cron.");
    }
}
