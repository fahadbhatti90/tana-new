...
namespace App\Model\Ams\AdGroup\SB;
...
class SBAdGroupReportId extends Model
{
    protected $table = 'tbl_ams_ad_group_sb_report_id';
        ...
        DB::transaction(function () use ($storeArray) {
            foreach ($storeArray as $row) {
                ...
                if ($existData->isEmpty()) {
                    ...
                    DB::table('tbl_ams_ad_group_sb_report_link')->where([
                        ['profile_id', $row['profile_id']],
                        ['report_date', $row['report_date']]
                    ])->delete();
                    try {
