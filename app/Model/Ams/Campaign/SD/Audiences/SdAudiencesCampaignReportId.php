...
class SdAudiencesCampaignReportId extends Model
{
    protected $table = 'tbl_ams_campaign_sd_audiences_report_id';
        ...
        DB::transaction(function () use ($storeArray) {
            foreach ($storeArray as $row) {
                ...
                if ($existData->isEmpty()) {
                    ...
                    DB::table('tbl_ams_campaign_sd_audiences_report_link')->where([
                        ['profile_id', $row['profile_id']],
                        ['report_date', $row['report_date']]
                    ])->delete();
                    try {
