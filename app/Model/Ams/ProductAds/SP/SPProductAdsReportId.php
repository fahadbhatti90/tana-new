...
namespace App\Model\Ams\ProductAds\SP;

use Illuminate\Database\Eloquent\Model;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Log;

class SPProductAdsReportId extends Model
{
    protected $table = 'tbl_ams_product_ads_sp_report_id';
        ...
        DB::transaction(function () use ($storeArray) {
            foreach ($storeArray as $row) {
                ...
                if ($existData->isEmpty()) {
                    ...
                    DB::table('tbl_ams_product_ads_sp_report_link')->where([
                        ['profile_id', $row['profile_id']],
                        ['report_date', $row['report_date']]
                    ])->delete();
                    try {
