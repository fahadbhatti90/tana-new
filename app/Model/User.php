...
class User extends Authenticatable
{
    ...
    /**
     * Get the user assigned permission.
     */
    public function authorization()
    {
        return $this->hasMany('App\Model\UserModulePermission','fk_user_id');
    }
     ...
     * @return mixed
     */
    public function getUserBrandProfile(){
        $vendors = $this->getUserSelectedBrandVendor()->where('is_active', 1)->pluck('vendor_id');
        $profile_id = DB::table('mgmt_vendor_entity')->whereIn('fk_vendor_id',$vendors)->pluck('fk_profile_id');
        return Profile::whereIN('id', $profile_id)->get();
    }
     ...
     * Get the Deleted Assigned Vendor for this User.
     */
    function getUserSelectedBrandVendor()
    {
        $fk_vendor_id = DB::connection('mysql2')->table('br_brand_vendor')->where('rdm_brand_id',  Session()->get('brand_id'))->pluck('rdm_vendor_id')->all();
        if($this->roles()->get()->first()->role_id == 3){
            $data = Vendors::whereIn('vendor_id', $fk_vendor_id)->where('is_active', 1)->get();
        }else{
            $data = Vendors::whereIn('vendor_id', $fk_vendor_id)->where('is_active', '!=',2)->get();
        }
        return $data;
    }
