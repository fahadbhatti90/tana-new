document.addEventListener('DOMContentLoaded', function (e) {
    ...
    var validationForm = FormValidation.formValidation(
        document.getElementById('salesOrder_form'),
        {
            fields: {
                vendor: {
                    validators: {
        ...
        var action_method = '';
        var namesfile = '';
        if ($('#action_button').val() == 'Upload in Sales Order') {
        ...
        var formData = new FormData($("#salesOrder_form").get(0));
        $.noConflict();
        $.ajax({
            url: base_url + "/salesOrder/store",
            type: 'POST',
            data: formData,
            dataType: 'json',
            processData: false,
            ...
            success: function (data) {
                ...
                if (data.error) {
                    ...
                    html += '</div>';
                    $("#salesOrderFile").val(null);
                    $('#form_result').html(html);
                    // $("#action_button").attr("disabled", false);
                    $("#action_button").attr("hidden", true);
                    $("#action_button_loader").attr("hidden", false);
                    document.getElementById("action_button").value = "Upload in Sales Order";
    ...
    $('output').on('click', '.del', function () { //to remove from front end and id as well
        ...
        id = id.split("_");
        id = id[1];
        var x = document.getElementById("salesOrderFile").files;
    ...
    function handleFileSelect(evt) {
        ...
        for (var i = 0, f; f = files[i]; i++) {
            ...
            reader.onload = (function (theFile, num) {
                return function (e) {
                    if (theFile.name.includes("Sales")) {
    ...
    document.getElementById('salesOrderFile').addEventListener('change', handleFileSelect, false);
...
function hide(x) {
    ...
    $('#target_div').append(
        $('<input/>').attr('type', "file").attr('name', "salesOrderFile[]").attr('accept', ".csv,.xlsx").attr('id', "salesOrderFile").attr('class', "custom-file-input").attr('multiple', "multiple").attr('required', "required").attr('onchange', "hide(this)")
    );
...
$('#salesOrderFile').change(function () {
    var i = this.files.length;
    document.getElementById("count").value = i;

    document.getElementById("salesOrderFile").style.visibility = "hidden";
    var myClock = document.getElementById('salesOrderFile');
    myClock.style.display = 'none';
    $('#target_div').append(
        $('<input/>').attr('type', "file").attr('name', "salesOrderFile[]").attr('accept', ".csv,.xlsx").attr('id', "salesOrderFile").attr('class', "custom-file-input").attr('multiple', "multiple").attr('required', "required").attr('onchange', "hide(this)").attr('onmouseout', "getvalue(this)")
    );
