document.addEventListener('DOMContentLoaded', function (e) {
    ...
    $('#salesOrder_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: base_url + "/salesOrder/verify_all",
        },
        language: {
    ...
    $(document).on('click', '.removeVendor', function () {
        ...
        $.ajax({
            url: base_url + "/salesOrder/destroy/" + id + "",
            type: "PUT",
            dataType: "json",
            success: function (data) {
                //set user data in model and show model
                if (data.success) {
                    $('#salesOrder_table').DataTable().ajax.reload();
                    Swal.fire({
                        title: "Done",
                        text: data.success,
                        type: "success",
    ...
    $('#move').click(function (e) {
        ...
        var rowCount = $('#salesOrder_table tbody tr').html().length;
        if (rowCount == 72) {
            e.preventDefault();
        }
