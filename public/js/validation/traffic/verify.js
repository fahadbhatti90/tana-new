document.addEventListener('DOMContentLoaded', function (e) {
    ...
    $('#verify_traffic_vendors_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: base_url + "/traffic/verify/" + vendor_id,
        },
        language: {
    ...
    $(document).on('click', '.removeVendor', function () {
        ...
        $.ajax({
            url: base_url + "/traffic/destroyDate/" + vendor_id + "",
            type: "PUT",
            data: data,
            dataType: "json",
            success: function (data) {
                ...
                if (data.errors) {
                    $('#verify_traffic_vendors_table').DataTable().ajax.reload();
                    Swal.fire({
